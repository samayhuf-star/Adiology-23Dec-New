-- Apply Workspace Migration to Production Database
-- Run this in Supabase Dashboard → SQL Editor

-- First check if workspaces table already exists
DO $$
BEGIN
    IF NOT EXISTS (SELECT FROM information_schema.tables WHERE table_name = 'workspaces') THEN
        -- Apply the full workspace migration
        ...
        CREATE INDEX idx_workspaces_is_admin ON workspaces(is_admin_workspace);
        ...
        CREATE INDEX idx_workspace_modules_module_name ON workspace_modules(module_name);
    ...
    ELSE
        RAISE NOTICE 'Workspace tables already exist';
    END IF;
END $$;
...
SELECT 'Workspace migration completed successfully!' as result;