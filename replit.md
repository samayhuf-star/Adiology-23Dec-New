...
- **UI/UX**: Component-based architecture, multi-step wizards for Campaign and Ads Builders, client-side routing, React hooks, and context-based notification system. Includes a SuperAdmin Console, an AI-powered Web Template Editor, Real-time Expense Tracking, and Call Forwarding. Mobile-responsive design with: collapsible sidebar navigation on mobile (hamburger menu), responsive grids that stack on smaller screens, compact navigation elements with scrollable step indicators, and mobile-optimized data tables with inline metrics display.
- **Call Forwarding (CallForwarding.tsx)**: Project-based call management system with SkySwitch API integration. Features: Project management (create, rename, delete projects), Tracking numbers with DID/TFN types, Multiple forwarding targets per tracking number with percentage-based call distribution, Visual progress bars showing percentage allocation, Sync to SkySwitch for production routing. Database tables: call_forwarding_projects, call_forwarding_tracking_numbers, call_forwarding_targets. API endpoints: `/api/call-forwarding/projects/*`, `/api/call-forwarding/tracking-numbers/*`, `/api/call-forwarding/targets/*`.
- **Call Forwarding Billing (CallForwardingBilling.tsx)**: Prepaid balance system with Stripe integration. Pricing: $3/number/month, $0.01/incoming call, $0.02/outgoing call. Features: Balance display with low-balance warning, auto-recharge ($25 when balance drops below $5), manual top-up, payment method management via Stripe, transaction history, and usage tracking. Database tables: call_forwarding_billing_accounts, call_forwarding_balance_transactions, call_forwarding_call_usage, call_forwarding_monthly_charges. API endpoints: `/api/call-forwarding/billing/*`.
...
- **SkySwitch**: Call forwarding and phone number management with OAuth2 authentication. Manages DIDs (Direct Inward Dial) and TFNs (Toll-Free Numbers) with percentage-based call distribution to multiple targets.
