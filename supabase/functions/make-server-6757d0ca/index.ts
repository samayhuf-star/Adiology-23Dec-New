...
async function verifySuperAdmin(c: any): Promise<{ valid: boolean; userId?: string; error?: string }> {
  try {
    ...
    // Allow test admin user (admin@admin.com) for development
    if (user.email === "admin@admin.com") {
      console.log("Test admin user verified:", user.id);
      return { valid: true, userId: user.id };
    }

    // Check database for superadmin role
    try {
      ...
      console.warn("Could not check database role (users table may not exist):", dbError);
    }

    console.error(`User ${user.id} (${user.email}) does not have superadmin role`);
